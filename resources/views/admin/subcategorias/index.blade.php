...
<div class="content-header">
    <div class="container-fluid">
        <div class="row mb-2">
            <div class="col-sm-6">
                <h1 class="m-0">Administración Subcategorias</h1>
            </div><!-- /.col -->
            <div class="col-sm-6">
                <ol class="breadcrumb float-sm-right">
                    <li class="breadcrumb-item"><a href="{{ route('dashboard') }}">Dashboard</a></li>
                    <li class="breadcrumb-item active">Administración Subcategorias</li>
                </ol>
            </div><!-- /.col -->
        </div><!-- /.row -->
    </div><!-- /.container-fluid -->
                ...
                <div class="modal fade" id="exampleModal" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
                    <div class="modal-dialog">
                        <div class="modal-content">
                            <form action="{{ route('subcategorias.store') }}" method="post" id="form-add" enctype="multipart/form-data">
                                @csrf
...
<script>
    ...
    function getSubcategorias(page) {
        $.get('/getSubcategorias', {}, function(data) {
    ...
    $(document).on('keyup', '.search', function() {
        if ($(this).val().length > 0) {
            var search = $(this).val();
            $.get('/getSubcategorias', { search: search }, function(data) {
                $('#all-data').html(data.result);
                $('#pagination-links').html(data.links)
            }, 'json');
            return;
    ...
    function paginarDepartamentos(page) {
        ...
        $.get('/getSubcategorias', { page: page, search: search }, function(data) {
            $('#all-data').html(data.result);
            $('#pagination-links').html(data.links);
        }, 'json');
    }
